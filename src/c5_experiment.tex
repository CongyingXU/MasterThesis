...
\section{实验环境搭建}
 本文作者使用Python 3.6搭建实验平台，并完成工具\tool 的开发，\tool 共包括xxx行代码。本文的实验评估在包含xx核xxx内存服务器上进行，随机抽样200个
 实验评估中，\tool 运行在包含xx核xxx内存服务器上。基于随机抽样的200个漏洞的运行结果，表明 \tool 完成一次漏洞补丁定位的时间约为xx分钟。
