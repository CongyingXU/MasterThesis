...
\end{enumerate}
 %43.8 15.1\% 41.1
这些结果体现了开源软件漏洞与其补丁之间映射关系的多样性，超过\tocheck{40\%}的漏洞与其补丁具有一对多的映射关系。在后文设计自动化补丁查找方法时，应充分考虑该特征。
...
\begin{enumerate}
    ...
    \item [（2）]开源软件漏洞补丁在类型、映射关系方面的特殊性，设计自动化补丁查找方法时应充分考虑。体现在：（i）93.7\%的补丁都为GitHub代码提交的形式。（ii）开源软件漏洞与其补丁之间映射关系具有多样性，超过40\%的漏洞与其补丁具有一对多的映射关系
\end{enumerate}
